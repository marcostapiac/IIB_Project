...
def getStateCovarianceMatrix(Theta, t1, t2, alpha, truncation, Sc):
    ...
    Sigma_c *= const
    return np.array(Sc + Sigma_c + 1e-320*np.eye(Sc.shape[0]))  # Partial Gaussian Approximation
...
def RB_Filter(time_index, a_updates, C_updates, log_weights, linear_positions, var_W, N_p, truncation,
    ...
    for i in range(N_p):
        ...
        linear_positions[time_index][i] = ss_A @ linear_positions[time_index - 1][i] + B @ np.matmul(
            np.linalg.cholesky(Ce), np.random.normal(size=P)).reshape(
            (P, 1))
        """ Kalman Filter"""
        E_N1 = E_Ns[i]
    ...
    position_estimates[time_index] = position_estimates[time_index]
    velocity_estimates[time_index] = velocity_estimates[time_index]
    mean_estimates[time_index] = mean_estimates[time_index]
    return a_updates, C_updates, N_p, E_Ns, log_weights, linear_positions, position_estimates, velocity_estimates, mean_estimates
...
def Langevin_2D():
    ...
    alpha = 1.99  # Determines Levy heavy-tailed-ness
    ...
    C_update += + 1e-320 * np.eye(C_update.shape[0])  # Make positive definite
    time_ax = np.linspace(0, T_horizon, 2000)  # Generate time-axis during which we are tracking
    ...
    linear_positions = np.array([[a_update] * N_p] * L)
    position_estimates = [0] * L
    velocity_estimates = [0] * L
    mean_estimates = [0] * L
    """ Run RBP Filter """
    for i in tqdm(range(1, L)):
        a_updates, C_updates, N_p, E_Ns, log_weights, linear_positions, position_estimates, velocity_estimates, mean_estimates \
            = RB_Filter(i, a_updates, C_updates, log_weights, linear_positions, var_W, N_p, truncation, time_ax[i - 1],
                        time_ax[i],
                        alpha, h, Theta, alpha_W, beta_W, sde_A, E_Ns, H, Cv, B, Observations[i], position_estimates,
                        velocity_estimates, mean_estimates)
    """ Plot results """
    plotFilterResults(time_ax, position_states, position_estimates, velocity_states, velocity_estimates, mean_states,
                      mean_estimates, linear_positions, a_updates, var_W * C_updates, log_weights)


def plotFilterResults(time_ax, position_states, position_estimates, velocity_states, velocity_estimates, mean_states,
                      mean_estimates, linear_positions, a_updates, C_updates, log_weights):
    fig, ax = plt.subplots()
    ax.plot(time_ax, position_states, linestyle="dashed", color='black', label="Real Position")
    ax.plot(time_ax, position_estimates, linestyle="dotted", color='orange', label="Estimated Position")
    ax.set_title("Alpha-Stable Position Particle Filtering")
    ax.set_xlabel("Time, $t$")
    ax.set_ylabel("Position, $x(t)$")
    plt.legend()
    fig, ax = plt.subplots()
    ax.plot(time_ax, velocity_states, linestyle="dashed", color='black', label="Real Velocity")
    ax.plot(time_ax, velocity_estimates, linestyle="dotted", color='orange', label="Estimated Velocity")
    ax.set_title("Alpha-Stable Velocity Particle Filtering")
    ax.set_xlabel("Time, $t$")
    ax.set_ylabel("Velocity, $\dot{x}(t)$")
    plt.legend()
    fig, ax = plt.subplots()
    ax.plot(time_ax, mean_states, linestyle="dashed", color='black', label="Real Mean")
    ax.plot(time_ax, mean_estimates, linestyle="dotted", color='orange', label="Estimated Mean")
    ax.set_title("Alpha-Stable Mean Particle Filtering")
    ax.set_xlabel("Time, $t$")
    ax.set_ylabel("Process Mean, $\mu_{W}$")
    plt.legend()
    """ Plot posterior density """
    L = len(position_estimates)
    posterior = np.zeros(L)
    for i in tqdm(range(L)):
        for j in range(len(log_weights)):
            posterior[i] += norm.pdf(velocity_estimates[i], loc=np.exp(log_weights[j]) * a_updates[j][1, 0],
                                     scale=np.exp(log_weights[j]) * np.sqrt(C_updates[j][1, 0]))
    fig, ax = plt.subplots()
    ax.scatter(position_estimates, posterior, s=8, color="black", label="Estimated Posterior Density")
    ax.set_title("Alpha-Stable Posterior Density of Position")
    ax.set_xlabel("Hidden State 1: Position")
