...
def normalGammaLatentSampling(t1, t2, nu, gamma, truncation, P, Theta):
    """ Aim to efficiently compute observation and latent state marginals and posteriors """
    time = t2
    delta_t = t2 - t1
    # Generate jump times from Gamma Process
    epochs = np.cumsum(np.random.exponential(scale=1.0 / delta_t, size=100))  # scale = 1/delta_t
...
def getNormalGammaStateMatrix(Theta, nu, gamma, mc, P, subordinator_truncation, delta_t):
    ...
    assert (ss_A[0, 2] == (mc - Yc)[0, 0])
    assert (ss_A.shape == (P + 1, P + 1))
    return ss_A


def getNormalGammaCovarianceMatrix(Theta, t1, t2, nu, gamma, subordinator_truncation, Sc, var_W):
    delta_t = t2 - t1
...
def RB_Filter(time_index, a_updates, C_updates, a_predicts, C_predicts, log_weights, var_W, N_p, nu, gamma, truncation,
    ...
    for i in range(N_p):
        if Mcs:
            ...
            mc, Sc = normalGammaLatentSampling(t1, t2, nu, gamma, truncation, P, Theta)
        ...
        Ce = getNormalGammaCovarianceMatrix(Theta, t1, t2, nu, gamma, subordinator_truncation, Sc, 1)
        """ Kalman Filter"""
        E_N1 = E_Ns[i]
...
def Langevin_2D(simulate_data=True):
    ...
    for i in tqdm(range(1, len(time_ax))):
        a_updates, C_updates, a_predicts, C_predicts, N_p, E_Ns, log_weights, position_estimates, velocity_estimates, skewness_estimates, position_stds, velocity_stds, skewness_stds = \
            RB_Filter(i, a_updates, C_updates, a_predicts, C_predicts, log_weights, var_W, N_p, nu, gamma, truncation,
                      subordinator_truncation,
                      time_ax[i - 1], time_ax[i], Theta, alpha_W, beta_W, E_Ns, H, Cv, B, Observations[i-1],
...
def langevinParameters():
    Theta = -0.1  # Mean reversion term -> small theta means jump lasts longer
...
def gammaParameters():
    ...
    truncation = nu * exp1(((gamma ** 2) / 2) * subordinator_truncation)  # Epoch size cutoff
