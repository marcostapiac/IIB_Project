...
def test_LGSSM_KF():
    ...
    for i in range(1, len(time_ax)):
        x = A @ states[i - 1] + np.random.multivariate_normal(mean=np.zeros(Q.shape[0]), cov=Q)
        y = H @ x + np.random.normal(loc=0, scale=R)
        states.append(x)
        observations.append(y)
        mean_pred, cov_pred = predict(mean_update, cov_update, A, Q, 0, 0)
        mean_update, cov_update = update(mean_pred, cov_pred, observations[i], R, H)
        #mean_pred, cov_pred, mean_update, cov_update, _, _ = KalmanFilter(A, B, np.eye(Q.shape[0]), Q, R, H,
        #                                                                  mean_update, cov_update, y, u)
        preds.append(mean_update[0])

    position = []
    for i in range(len(states)):
        position.append(states[i][0])
    plt.plot(time_ax, preds, label="Kalman Estimate", linestyle='dashed', color='r')
    plt.plot(time_ax, position, label="State", color='b')
