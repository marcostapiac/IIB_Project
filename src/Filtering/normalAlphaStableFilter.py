...
def run_filter(alpha, gamma, delta, simulate_data=True):
    P = 2
    M = 1
    Theta = -1.0
    var_W = np.sqrt(1 ** 2) ** 2
    ...
    if simulate_data:
        time_ax = np.cumsum(
            np.random.exponential(scale=1.0 / T_horizon, size=L + 1))  # Generate time-axis during which we are tracking
        ...
        for i in tqdm(range(1, len(time_ax))):
            ...
            alphaSubordinator = AlphaSubordinator(t1, t2, nEpochs, nEpochs, subordinator_truncation, alpha, gamma, delta)
            mc, Sc = lModel.filter_sampling(alphaSubordinator)
            lModel.update_state_covariance_matrix(alphaSubordinator.get_covariance_constant(), t2 - t1, Sc)
    ...
    for i in tqdm(range(1, numObs)):
        subordinator = AlphaSubordinator(time_ax[i - 1], time_ax[i], nEpochs, nEpochs, subordinator_truncation,
                                         alpha, gamma, delta)
        a_predicts, C_predicts = rbf.filter(i, time_ax[i] - time_ax[i - 1], subordinator, Observations[i], lModel,
                                            a_predicts, C_predicts)

    plot_title = "Alpha-Stable Particle Filtering, $\\theta, N_{particles}, k_{V}, \\alpha, \sigma_{W}^{2}, k_{W} = " + str(
        Theta) + ", " + str(Np) + ", " + str(k_V) + ", " + str(alpha) + ", " + str(
        var_W) + ", " + str(
        k_W) + "$"
