...
from test_kalman import KalmanFilterNonObject
from scipy.special import gamma as gammaf
from scipy.special import gammaincc as uppergammaf
from scipy.special import gammainc as lowergammaf
import matplotlib.pyplot as plt
from ClassKalmanFilter import KalmanFilter
from data_processing import obtainData


def generate_ts_jumps(kappa, beta, C, T_horizon, subordination_truncation, N_epochs):
    epochs = np.cumsum(np.random.exponential(scale=1.0 / T_horizon, size=N_epochs))
    x = ((kappa * epochs) / C) ** (-1 / kappa)
    prob_acc = np.exp(-beta * x)
    us = np.random.uniform(0., 1., size=prob_acc.size)
    x = np.where(prob_acc > us, x, 0.)
    x = x * (x >= subordination_truncation)
    return x
...
def ntsLatentSampling(t1, t2, kappa, delta, gamma, subordination_truncation, N_epochs, P, Theta):
    ...
    C = delta * (2 ** kappa) * kappa * (1 / gammaf(1 - kappa))
    tsJumps = generate_ts_jumps(kappa, beta, C, T_horizon=delta_t, subordination_truncation=subordination_truncation,
                                N_epochs=N_epochs)
    Vs = np.random.uniform(t1, t2, size=tsJumps.shape[0])
    M1 = np.array([1 / Theta, 1]).reshape((P, 1))
    M2 = np.array([-1 / Theta, 0]).reshape((P, 1))
    mean_sum_1 = np.sum(tsJumps * (np.exp(Theta * (time - Vs))))
    mean_sum_2 = np.sum(tsJumps)
    ...
    assert (M3[1, 1] == 0)
    cov_sum_1 = np.sum(tsJumps * np.exp(2 * Theta * (time - Vs)))
    cov_sum_2 = np.sum(tsJumps * np.exp(Theta * (time - Vs)))
    cov_sum_3 = np.sum(tsJumps)
...
def getNTSStateMatrix(Theta, kappa, delta, gamma, mc, P, subordinator_truncation, delta_t):
    const = 2 * delta * kappa * (gamma ** ((1 - kappa) / kappa)) * lowergammaf(1 - kappa,
...
def getNTSCovarianceMatrix(Theta, t1, t2, kappa, delta, gamma, subordinator_truncation, Sc):
    delta_t = t2 - t1
    # Partial Gaussian approximation of residual
    const = 4 * delta * kappa * (gamma ** ((2 - kappa) / kappa)) * (
...
def RB_Filter(time_index, a_updates, C_updates, a_predicts, C_predicts, log_weights, var_W, N_p, kappa, delta, gamma,
    ...
    for i in range(N_p):
        if Mcs:
            ...
            mc, Sc = ntsLatentSampling(t1, t2, kappa, delta, gamma, subordinator_truncation, N_epochs, P,
                                       Theta)
        """ Given proposed mc, Sc, we can now define Kalman Filter matrices """
        ss_A = getNTSStateMatrix(Theta, kappa, delta, gamma, mc, P, subordinator_truncation, delta_t)
        Ce = getNTSCovarianceMatrix(Theta, t1, t2, kappa, delta, gamma, subordinator_truncation, Sc)
        """ Kalman Filter"""
        E_N1 = E_Ns[i]
        a_predict, C_predict, a_update, C_update, F_N, E_N = KalmanFilterNonObject(ss_A, B, Ce, Cv, H,
                                                                          a_updates[i],
...
def Langevin_2D(simulate_data=True):
    ...
    kappa, delta, gamma, subordinator_truncation = subordinatorParameters()
    ...
    if simulate_data:
        ...
        time_ax = np.linspace(0, T_horizon, 200)  # Generate time-axis during which we are tracking
        ...
        for i in tqdm(range(1, len(time_ax))):
            ...
            t2 = time_ax[i]
            delta_t = t2 - t1
            mc, Sc = ntsLatentSampling(t1, t2, kappa, delta, gamma, subordinator_truncation, N_epochs, P, Theta)
            ss_A = getNTSStateMatrix(Theta, kappa, delta, gamma, mc, P, subordinator_truncation, delta_t)
            Ce = getNTSCovarianceMatrix(Theta, t1, t2, kappa, delta, gamma, subordinator_truncation, Sc)
    ...
    for i in tqdm(range(1, len(time_ax))):
        a_updates, C_updates, a_predicts, C_predicts, N_p, E_Ns, log_weights, position_estimates, velocity_estimates, skewness_estimates, position_stds, velocity_stds, skewness_stds = \
            RB_Filter(
                i, a_updates, C_updates, a_predicts, C_predicts, log_weights, var_W, N_p, kappa, delta, gamma,
    ...
    if simulate_data:
        plotFilterResults(time_ax, position_states, position_estimates, position_stds, velocity_states,
                          ...
                          skewness_estimates, skewness_stds, Theta, N_p, k_V, kappa, delta, gamma, var_W, k_W)
    else:
...
def langevinParameters():
    ...
    Theta = -0.01  # Friction constant / speed reversion term
    ...
    Cv = k_V
    mu_mu_W = 1  # np.mean(observations)  # Mean prior mean
...
def subordinatorParameters():
    ...
    delta = 1
    gamma = 1.26
    subordinator_truncation = 0.01  # Truncation on GIG Jumps
    return kappa, delta, gamma, subordinator_truncation
...
def plotFilterResults(time_ax, position_states, position_estimates, position_stds, velocity_states, velocity_estimates,
    ...
    ax[0].set_title(
        "Normal Tempered Stable Particle Filtering, $\\theta, N_{particles}, k_{V}, \kappa, \delta, \gamma, \sigma_{W}^{2}, "
        "k_{W} = " + str(Theta) + ", " + str(N_p) + ", " + str(k_V) + ", " + str(kappa) + ", " + str(
