...
def verify_simulation(t1=0.0, t2=1.0, num_obs=100, num_epochs=2000, subordinator_truncation=0.0, mu=0., mu_W=0.0,
    ...
    title = "Q-Q Plot for Normal Gamma Stable Process with $\\mu, \\mu_{W}, \\sigma_{w}, \\nu, \\gamma = " + str(
        mu) + " ," + str(mu_W) + " ," + str(var_W) + " ," + str(nu) + " ," + str(
        (t2 - t1) * gamma) + " ," + "$ at time $t = T_{horizon}$"
    plotter = QQPlotter(rvs, proc_vals, ylabel="Marginal Random Variables of Normal Gamma Stable Process", log=False,
                        plottitle=title)
    plotter.plot()
    plt.show()
