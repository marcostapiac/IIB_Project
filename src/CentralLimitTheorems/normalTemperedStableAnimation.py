...
def prepare_animation():
    def animate(frame_number):
        ...
        sub = TemperedStableSubordinator(t1, t2, num_obs, num_epochs, subordinator_truncation=subordinator_truncation, kappa=kappa, delta=delta,
                                gamma=gamma)
        ...
        title = "Histogram for Residual Normal Tempered Stable with $\\mu, \\mu_{W}, \\sigma_{w}, \\kappa, \\delta, \\gamma = " + str(
            mu) + " ," + str(mu_W) + " ," + str(var_W) + " ," + str(kappa) + " ," + str((t2 - t1) * delta) + " ," + str(
            gamma) + "$ at time $t = T_{horizon}$"
