...
def verify_CLT(t1=0.0, t2=1.0, num_obs=200, num_epochs=2000, subordinator_truncation=0.0, mu=0., mu_W=0.0,
               var_W=1.0, delta=0.8, gamma=1.0, lambd=-0.4, nProcesses=10):
    sub = GIGSubordinator(t1, t2, num_obs, num_epochs, subordinator_truncation=subordinator_truncation, delta=delta,
                          gamma=gamma,
                          lambd=lambd)
    ...
    title = "Histogram for Residual Generalised Hyperbolic with $\\mu, \\mu_{W}, \\sigma_{w}, \\delta, \\gamma, " \
            "\\lambda = " + str(
        mu) + " ," + str(mu_W) + " ," + str(var_W) + " ," + str(delta) + " ," + str(gamma) + " ," + str(
        delta) + "$ at time $t = T_{horizon}$"
    ...
    plotter.plot_normal()
    plt.show()
    # plt.savefig("GHCLTHistogram.pgf", bbox_inches='tight')


verify_CLT(subordinator_truncation=1e-4, nProcesses=10000)
